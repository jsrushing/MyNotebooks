...
namespace MyNotebooks.objects
{
	...
	{
		public Group(DataTable dt, int rowIndex) { PopulateFromDataTable(dt, rowIndex); }
				...
				if (!sPropertyName.Name.ToLower().Contains("orglevel"))
				{
					...
					catch (Exception ex)
					{
						if (ex.GetType() != typeof(InvalidCastException))
						{
							...
							"' occurred while processing the Group property '" + sPropertyName + "'.", "Error Occurred")) { frm.ShowDialog(); }
						}
					}
				}
			}
